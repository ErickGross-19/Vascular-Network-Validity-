...
# Cell 4d: inlet/outlet guess + Poiseuille network (Part D)
def compute_poiseuille_network(
    ...
    ----------
    G : networkx.Graph
        Centerline graph with at least 'radius' (node or edge) and
        optionally 'length' (edge) and 'coord' (node, 3D coordinate).
    mu : float
        Dynamic viscosity.
    inlet_nodes, outlet_nodes : list or None
        Node IDs for inlets and outlets. If both are None, they are
        auto-detected from leaf nodes using their Z coordinate.
    pin, pout : float
        Boundary pressures at inlets and outlets.
    ...
    results : dict
        {
          ...
          "edge_flows": {(u, v): flow_rate, ...},
          "summary": {...},
          "G": G  # same graph, possibly with attributes written
        }
    """
    ...
    if len(components) > 1:
        msg = ("Graph has multiple connected components. "
               "Components without any boundary node will be under-constrained.")
        warnings_list.append(msg)

    # ------------------------------------------------------------------
    # 1) Precompute edge conductances
    # ------------------------------------------------------------------
    edge_conductance = {}

    def compute_edge_conductance(u, v):
        data = G.edges[u, v]

        # First try node radii
        ru = G.nodes[u].get("radius", None)
        rv = G.nodes[v].get("radius", None)
        r_edge = data.get("radius", None)

        if ru is None and rv is None and r_edge is None:
            return 0.0
        ...
        if length is None:
            cu = G.nodes[u].get("coord", None)
            cv = G.nodes[v].get("coord", None)
        ...
        # Poiseuille conductance
        G_nm = (pi * (r ** 4)) / (8.0 * mu * length)
        return G_nm

    for u, v in G.edges:
        G_uv = compute_edge_conductance(u, v)
        edge_conductance[(u, v)] = G_uv
        edge_conductance[(v, u)] = G_uv

    # ------------------------------------------------------------------
    # 2) Boundary detection (inlets/outlets)
    # ------------------------------------------------------------------
    leaves = [n for n in G.nodes if G.degree(n) == 1]

    if inlet_nodes is None and outlet_nodes is None:
        if not leaves:
            msg = ("No leaf nodes found for auto-detecting boundaries. "
                   "All nodes will be treated as interior; "
                   "the linear system is likely singular.")
            warnings_list.append(msg)
            inlet_nodes = []
            outlet_nodes = []
        else:
            # Use Z coordinate to split leaves into "bottom" (inlets) and "top" (outlets)
            leaves_with_z = []
            for n in leaves:
                coord = G.nodes[n].get("coord", None)
                if coord is None:
                    z = 0.0
                else:
            ...
            half = max(1, len(leaves_with_z) // 2)
            inlet_nodes = [n for n, _ in leaves_with_z[:half]]
            outlet_nodes = [n for n, _ in leaves_with_z[half:]]
    else:
    ...
    if not inlet_nodes or not outlet_nodes:
        msg = ("Missing inlets or outlets after validation. "
               "The linear system may be under-constrained or singular.")
        warnings_list.append(msg)
    ...
    if n_int > 0:
        idx_int = {n: i for i, n in enumerate(interior_nodes)}
        ...
        for n in interior_nodes:
            ...
            for m in G.neighbors(n):
                ...
                if m in interior_nodes:
                    ...
                    P_m = boundary_pressure.get(m, 0.0)
                    A[i, i] += G_nm
                    b[i] += G_nm * P_m

        rank_A = int(np.linalg.matrix_rank(A))
        if rank_A < A.shape[0]:
            msg = "Matrix is rank-deficient; using least-squares solution."
            warnings_list.append(msg)

        try:
            P_int = np.linalg.solve(A, b)
        except np.linalg.LinAlgError:
            msg = "Singular matrix in np.linalg.solve; falling back to np.linalg.lstsq."
            warnings_list.append(msg)
        ...
        for n in interior_nodes:
            i = idx_int[n]
            node_pressures[n] = float(P_int[i])
        ...
        node_pressures = {}
        for n in G.nodes:
            if n in boundary_pressure:
                node_pressures[n] = float(boundary_pressure[n])
            else:
                node_pressures[n] = float(pout)
        matrix_shape = (0, 0)
        matrix_rank = 0
    ...
    # ------------------------------------------------------------------
    # 5) Optionally write results back to graph
    # ------------------------------------------------------------------
    if write_to_graph:
        # Node pressures
...
def ensure_poiseuille_solved(G, delta_p: float = 1.0, mu: float = 1.0e-3):
    """
    Ensure that Poiseuille solution is attached to G.

    - If any node already has a 'pressure' attribute, G is returned unchanged.
    - Otherwise, it automatically chooses inlet/outlet as two degree-1 nodes,
      runs compute_poiseuille_network, and writes 'pressure' (nodes) and
      'Q' (edges) into G.

    Returns
    -------
    G : networkx.Graph
        The same graph, with Poiseuille fields attached.
    """
    # Already solved?
    if any("pressure" in data for _, data in G.nodes(data=True)):
        return G

    # Pick boundary nodes: degree-1 endpoints
    boundary_nodes = [n for n in G.nodes if G.degree[n] == 1]
    if len(boundary_nodes) < 2:
        print("[ensure_poiseuille_solved] Not enough boundary nodes to pick inlet/outlet.")
        return G

    inlet = boundary_nodes[0]
    outlet = boundary_nodes[1]

    print(
        f"[ensure_poiseuille_solved] Solving Poiseuille with "
        f"inlet={inlet}, outlet={outlet}, Δp={delta_p} Pa, mu={mu}"
    )

    _ = compute_poiseuille_network(
        G,
        mu=mu,
        inlet_nodes=[inlet],
        outlet_nodes=[outlet],
...
# --- Helper to get node coordinates robustly ---

def get_node_pos(G, n):
    """
    Return (x, y, z) position of node n.
    Tries 'pos', then 'coord', then 'xyz'.
    ...
    if "pos" in data:
        ...
        raise KeyError(
            f"Node {n} has no 'pos', 'coord', or 'xyz' attribute. "
...
# --- Updated 2D centerline plotting ---

def plot_centerline_graph_2d(G, plane="xz", title="Centerline graph (2D)"):
    """
    Plot the centerline graph projected to a 2D plane (xy, xz, or yz).
    Uses get_node_pos to be robust to attribute names.
    """
    plane = plane.lower()
    if plane == "xy":
        i, j = 0, 1
    ...
    for u, v in G.edges():
        p0 = get_node_pos(G, u)
        p1 = get_node_pos(G, v)
        segments.append([[p0[i], p0[j]], [p1[i], p1[j]]])

    lc = LineCollection(segments, linewidths=1.5)
...
# --- Updated scalar-on-centerline plotting ---
...
def plot_centerline_scalar(
    ...
    """
    Color each edge by a scalar attribute (e.g. Q, pressure drop).
    Uses get_node_pos to be robust to attribute names.
    """
    segments = []
    values = []
    ...
    for u, v, data in G.edges(data=True):
        ...
        if log_abs:
            val = np.log10(np.abs(val) + 1e-16)
        values.append(val)

    segments_2d = [np.array([[p0[0], p0[2]], [p1[0], p1[2]]]) for p0, p1 in segments]
    ...
    cbar.set_label(edge_attr if not log_abs else f"log10|{edge_attr}|")
    ax.set_xlabel("x")
    ax.set_ylabel("z")
    ax.set_title(title)
    plt.tight_layout()
...
# ---------- B. Flow distribution across branches ----------
def plot_flow_distribution(G, edge_attr="Q", max_edges=None):
    """
    Bar plot of |Q| per edge, sorted descending.
    """
    flows = []
    edge_ids = []
    for idx, (u, v, data) in enumerate(G.edges(data=True)):
        Q = float(data.get(edge_attr, 0.0))
        flows.append(abs(Q))
        edge_ids.append(idx)

    flows = np.array(flows, dtype=float)
    order = np.argsort(-flows)  # descending

    flows_sorted = flows[order]
    edge_ids_sorted = np.array(edge_ids)[order]

    if max_edges is not None:
        flows_sorted = flows_sorted[:max_edges]
        edge_ids_sorted = edge_ids_sorted[:max_edges]

    fig, ax = plt.subplots(figsize=(6, 4))
    ax.bar(np.arange(len(flows_sorted)), flows_sorted)
    ax.set_xlabel("Edge rank (by |Q|)")
    ax.set_ylabel(f"|{edge_attr}|")
...
# ---------- C. Pressure profile along a path ----------
def plot_pressure_profile_along_path(G, node_path):
    """
    Plot pressure vs cumulative length along a given node path.
    node_path: list of node IDs in traversed order.
    """
    if len(node_path) < 2:
        raise ValueError("node_path must contain at least two nodes")

    # collect pressures and cumulative length
    p_vals = []
    s_vals = [0.0]

    total_s = 0.0
    for i, n in enumerate(node_path):
        p_vals.append(float(G.nodes[n].get("pressure", 0.0)))
        if i > 0:
            u = node_path[i - 1]
            v = n
            # assume an edge exists in either direction
            if G.has_edge(u, v):
                L = float(G.edges[u, v].get("length", 0.0))
            elif G.has_edge(v, u):
                L = float(G.edges[v, u].get("length", 0.0))
            else:
                L = 0.0
            total_s += L
            s_vals.append(total_s)

    s_vals = np.array(s_vals, dtype=float)
    p_vals = np.array(p_vals, dtype=float)

    fig, ax = plt.subplots(figsize=(6, 4))
    ax.plot(s_vals, p_vals, marker="o")
    ax.set_xlabel("Cumulative path length")
    ax.set_ylabel("Pressure (Pa)")
    ax.set_title("Pressure profile along selected path")
    plt.tight_layout()
    plt.show()
    return fig


# ---------- D. Histograms / distributions ----------
def plot_poiseuille_histograms(G):
    """
    Histograms of radius, hydraulic resistance, velocity, wall shear.
    Expects edges to have attributes:
        'radius_mean', 'R_hyd', 'u_mean', 'tau_wall'
    """
    radius = []
    R_hyd = []
    u_mean = []
    ...
    for u, v, data in G.edges(data=True):
        radius.append(float(data.get("radius_mean", np.nan)))
        R_hyd.append(float(data.get("R_hyd", np.nan)))
        u_mean.append(float(data.get("u_mean", np.nan)))
        tau_wall.append(float(data.get("tau_wall", np.nan)))

    radius = np.array(radius, dtype=float)
    R_hyd = np.array(R_hyd, dtype=float)
    u_mean = np.array(u_mean, dtype=float)
    tau_wall = np.array(tau_wall, dtype=float)

    fig, axes = plt.subplots(2, 2, figsize=(10, 8))
    ax = axes.ravel()

    ax[0].hist(radius[~np.isnan(radius)], bins=20)
    ax[0].set_title("Radius distribution")
    ax[0].set_xlabel("radius_mean")
    ax[0].set_ylabel("count")

    ax[1].hist(R_hyd[~np.isnan(R_hyd)], bins=20)
    ax[1].set_title("Hydraulic resistance R_hyd")
    ax[1].set_xlabel("R_hyd (Pa·s/m³)")
    ax[1].set_ylabel("count")

    ax[2].hist(u_mean[~np.isnan(u_mean)], bins=20)
    ax[2].set_title("Mean velocity")
    ax[2].set_xlabel("u_mean (m/s)")
    ax[2].set_ylabel("count")

    ax[3].hist(tau_wall[~np.isnan(tau_wall)], bins=20)
    ax[3].set_title("Wall shear stress")
    ax[3].set_xlabel("tau_wall (Pa)")
...
report, G_centerline = validate_and_repair_geometry("C:/Users/Erick/Downloads/Vascular_Network_Original.stl", voxel_pitch=0.01, smooth_iters=30, dilation_iters=1)
