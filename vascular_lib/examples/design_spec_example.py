...
from vascular_lib.specs import DesignSpec, TreeSpec, DualTreeSpec, EllipsoidSpec, BoxSpec, ColonizationSpec
...
def example_1_single_tree_with_preset():
    ...
    print(f"  Inlet: {spec.tree.inlets[0].position}")
...
def example_3_dual_tree_design():
    ...
    print(f"\n✓ Created DesignSpec with DualTreeSpec:")
    print(f"  Domain: Ellipsoid {spec.domain.semi_axes}")
...
def example_4_iterative_refinement():
    ...
    if eval_result.scores.coverage_score < 0.7:
        ...
        spec.tree.colonization = get_preset("liver_arterial_dense")
...
def example_5_spec_serialization():
    ...
    loaded_spec = DesignSpec.from_dict(spec_dict)
